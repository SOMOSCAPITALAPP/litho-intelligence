...
type RecommendationClientProps = {
  initialPayload: RecommendationPayload;
};

export function RecommendationClient({ initialPayload }: RecommendationClientProps) {
    ...
    <>
      <form className="form-panel" onSubmit={submit}>
        ...
        <div className="field">
          ...
          <input
            id="goal"
            placeholder="amour, abondance, confiance, protection, paix..."
            value={goal}
      ...
      <div className="premium-results-shell">
        ...
        {results.length > 1 ? (
          <section className="card">
            <RelatedStoneLinks items={results.map((item) => item.slug)} title="Autres pierres possibles" />
          </section>
        ) : null}
        {results.map((item) => (
      ...
      {results.length > 0 ? (
        <LeadCaptureCard
          ...
          recommendedStone={results[0]?.name}
          title="Recevoir ma recommandation complète"
          subtitle="Gardez votre pierre, son rituel et les bracelets recommandés pour y revenir plus tard."
